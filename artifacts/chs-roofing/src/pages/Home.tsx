...
import { ArrowRight, CheckCircle, Shield, Home, Wrench, Award, Star, Quote, ChevronRight, Clock, ShieldCheck, Phone, ChevronLeft, Sparkles, CloudLightning, Droplets, Paintbrush, Calculator } from "lucide-react";
...
export default function HomePage() {
  ...
  const services = SERVICES.map((s) => ({
    title: t(`services.${s.slug}.title`, { defaultValue: s.title }),
    desc: t(`services.${s.slug}.short`, { defaultValue: s.short }),
  ...
  const team = TEAM;
  const stormItems = t("stormBlock.items", { returnObjects: true }) as string[];
  const whyItems = t("whyChoose.items", { returnObjects: true }) as { title: string; desc: string }[];
    ...
    <>
      <main>
              ...
              >
                <div className="flex flex-wrap items-center gap-3 mb-7">
                  ...
                  <div className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-[hsl(var(--accent-gold))]/40 text-[hsl(var(--accent-gold))] text-[11px] font-semibold tracking-wider uppercase">
                    ...
                    {t("hero.ownership", { year: SITE.established })}
                  </div>
                </div>
                <h1 className="text-5xl md:text-6xl lg:text-7xl font-display font-bold text-white leading-[1.02] tracking-tight mb-6 drop-shadow-lg">
                  {t("hero.title")} <span className="text-primary drop-shadow-md">{t("hero.titleAccent")}</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    {t("common.getFreeQuote")} <ArrowRight className="w-4 h-4" />
                  </Link>
                  <a
                ...
                <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="flex flex-col gap-1 text-white">
                    ...
                    <span className="text-xs uppercase tracking-wider font-semibold opacity-80">{t("trust.yearsInBusiness")}</span>
                  </div>
                  <div className="flex flex-col gap-1 text-white">
                    <CountUp to={500} suffix="+" className="font-display text-3xl font-bold text-primary" />
                    <span className="text-xs uppercase tracking-wider font-semibold opacity-80">{t("trust.projectsCompleted")}</span>
                  </div>
                  <div className="flex flex-col gap-1 text-white">
                    <CountUp
                      to={5}
                    ...
                    />
                    <span className="text-xs uppercase tracking-wider font-semibold opacity-80">{t("trust.googleRated")}</span>
                  </div>
                  <div className="flex flex-col gap-1 text-white">
                    <CountUp
                      to={247}
                      ...
                      className="font-display text-3xl font-bold text-primary"
                    />
                    <span className="text-xs uppercase tracking-wider font-semibold opacity-80">{t("trust.emergencyResponse")}</span>
                  </div>
                </div>
              </motion.div>
          ...
          <div className="container mx-auto px-6">
            <div className="flex flex-wrap justify-center md:justify-between items-center gap-6 md:gap-4 text-center">
              <div className="flex items-center gap-2 text-foreground font-bold">
                <CheckCircle className="w-5 h-5 text-primary" /> {t("trust.license", { license: SITE.license })}
              </div>
              <div className="hidden md:block w-px h-6 bg-border"></div>
              <div className="flex items-center gap-2 text-foreground font-bold">
                <Shield className="w-5 h-5 text-primary" /> {t("trust.fullyInsured")}
              </div>
              <div className="hidden lg:block w-px h-6 bg-border"></div>
              <div className="flex items-center gap-2 text-foreground font-bold">
                <Award className="w-5 h-5 text-primary" /> {t("trust.bbbAccredited")}
              </div>
              <div className="hidden md:block w-px h-6 bg-border"></div>
              <div className="flex items-center gap-2 text-foreground font-bold">
                <Home className="w-5 h-5 text-primary" /> {t("trust.familyOwned")}
              </div>
              <div className="hidden lg:block w-px h-6 bg-border"></div>
              <div className="flex items-center gap-2 text-foreground font-bold">
                <Star className="w-5 h-5 text-yellow-500 fill-yellow-500" /> {t("trust.fiveStarGoogle")}
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <FadeIn>
                <div className="bg-primary/10 border-l-4 border-primary p-10 rounded-r-2xl backdrop-blur-sm">
                  ...
                  >
                    {t("stormBlock.cta")} <ArrowRight className="w-5 h-5" />
                  </button>
                </div>
              </FadeIn>
              <FadeIn delay={0.2}>
                <div className="relative h-[400px] rounded-xl overflow-hidden shadow-2xl border border-white/10">
                  <img loading="lazy" decoding="async" src={PHOTOS.tearOff} alt={t("stormBlock.imageAlt")} className="w-full h-full object-cover" />
        ...
        <section className="py-28 bg-background bg-wash-warm">
          <div className="container mx-auto max-w-7xl px-4">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <SectionEyebrow number="01" label={t("servicesSection.eyebrow")} className="mx-auto" />
        ...
        <section className="py-28 bg-muted bg-wash-cool relative">
          <div className="container mx-auto max-w-7xl px-4">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <FadeIn delay={0.2}>
                <div>
                  <SectionEyebrow number="02" label={t("whyChoose.eyebrow")} />
                  ...
                  <div className="space-y-6">
                    {whyItems.map((item, idx) => {
                      const Icon = [Wrench, Clock, ShieldCheck][idx] ?? Wrench;
                      return (
        ...
        <section className="py-28 bg-background">
          <div className="container mx-auto max-w-7xl px-4">
            <div className="text-center max-w-3xl mx-auto mb-14">
              <SectionEyebrow number="03" label={t("featuredProjects.eyebrow")} className="mx-auto" />
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold tracking-tight text-foreground leading-[1.05]">
            ...
            <div className="text-center mt-12 flex flex-wrap justify-center gap-3">
              ...
              >
                {t("featuredProjects.viewResidential")} <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
              ...
              >
                {t("featuredProjects.viewCommercial")}
              </Link>
              <Link
                href="/gallery/multifamily"
                className="inline-flex items-center gap-2 bg-card border border-border/60 text-foreground hover:border-primary/40 px-6 py-3 rounded-full font-semibold text-sm tracking-tight transition-all"
              >
                {t("featuredProjects.viewMultifamily")}
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="py-24 bg-muted overflow-hidden relative">
          ...
          <div className="container mx-auto max-w-7xl px-4 relative z-10">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <SectionEyebrow number="04" label={t("testimonialsSection.eyebrow")} className="mx-auto" />
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold tracking-tight text-foreground leading-[1.05]">
        ...
        <section className="py-28 bg-background">
          <div className="container mx-auto max-w-7xl px-4">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <SectionEyebrow number="05" label={t("team.eyebrow")} className="mx-auto" />
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold tracking-tight text-foreground leading-[1.05]">
                {t("team.title")}
              </h2>
              <p className="text-muted-foreground mt-6 text-lg leading-relaxed max-w-2xl mx-auto">
        ...
        <section className="py-16 bg-white border-y border-border/40">
          <div className="container mx-auto max-w-7xl px-4">
            <p className="text-center text-xs font-semibold text-muted-foreground uppercase tracking-[0.2em] mb-10">{t("trust.verifiedRatings")}</p>
            ...
            <div className="flex flex-wrap justify-center items-center gap-4 mb-2">
              ...
              <div className="flex items-center gap-3 border border-border/60 rounded-2xl px-5 py-3.5 shadow-sm bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("trust.googleRating")}</p>
              ...
              <div className="flex items-center gap-3 border border-border/60 rounded-2xl px-5 py-3.5 shadow-sm bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("trust.accreditedBusiness")}</p>
                  <p className="text-lg font-display font-bold text-[#006DA6]">{t("trust.aPlusRating")}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 border border-border/60 rounded-2xl px-5 py-3.5 shadow-sm bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("trust.manufacturerCertified")}</p>
              ...
              <div className="flex items-center gap-3 border border-border/60 rounded-2xl px-5 py-3.5 shadow-sm bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("trust.flCertified")}</p>
                  <p className="text-sm font-bold text-foreground">{t("trust.roofingContractor")}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 border border-border/60 rounded-2xl px-5 py-3.5 shadow-sm bg-card hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("trust.flStateLicensed")}</p>
                  <p className="text-sm font-bold text-foreground">{SITE.license}</p>
                </div>
              </div>
            </div>
                ...
                <div className="text-white max-w-xl">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 glass-surface-dark border border-white/10 p-5 rounded-2xl">
                      ...
                      <div>
                        <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-[0.2em]">{t("finalCta.callPromise.label")}</p>
                        <a href={`tel:${SITE.phoneTel}`} className="text-2xl font-bold text-white hover:text-primary transition-colors tracking-tight">
                          {SITE.phoneDisplay}
                        </a>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 glass-surface-dark border border-white/10 p-5 rounded-2xl">
                      ...
                      <div>
                        <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-[0.2em]">{t("finalCta.promise.label")}</p>
                        <p className="text-lg font-semibold text-white tracking-tight">
                          {t("finalCta.promise.body")}
                        </p>
                      </div>
                    </div>
                  </div>
